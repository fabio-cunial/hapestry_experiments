    ...
    for s,(sample_label,series) in enumerate(histograms.items()):
        for series_label,coord in axes_locations.items():
            ...
            if "reference" in sample_label:
                ...
                st = "--"
                lw = 1
            elif "jasmine_1" in sample_label:
                c = "#63D540"  # Dark green
                st = "-"
                lw = 3
            elif "jasmine_2" in sample_label:
                c = "#63D540"  # Dark green
                #c = "#63D540"  # Light green
                st = "--"
                lw = 1
            elif "panpop_1" in sample_label:
                c = "#791D91"  # Dark pink
                st = "-"
                lw = 3
            elif "panpop_2" in sample_label:
                c = "#791D91"  # Dark pink
                #c = "#965635"  # Light brown
                st = "--"
                lw = 1
            elif "truvari_1" in sample_label:
                c = "#3685D6"  # Dark blue
                st = "-"
                lw = 3
            elif "truvari_2" in sample_label:
                c = "#3685D6"  # Dark blue
                #c = "#559FDE"  # Light blue
                st = "--"
                lw = 1
            elif "pbsv_pop" in sample_label:
                c = "#F4D94A"  # Dark yellow
                st = "-"
                lw = 3
            elif "sniffles_pop" in sample_label:
                c = "#F4D94A"  # Dark yellow
                #c = "#791D91"  # Light pink
                st = "--"
                lw = 1
...
def plot_parallel_coordinate(data, data_ranges, title, colors, output_dir=None):
    ...
    for s,sample in enumerate(data.keys()):
        ...
        if "reference" in sample:
            ...
            st = "--"
            lw = 1
        elif "truvari_1" in sample:
            c = "#3685D6"  # Dark blue
            st = "-"
            lw = 3
        elif "truvari_2" in sample:
            c = "#3685D6"  # Dark blue
            #c = "#559FDE"  # Light blue
            st = "--"
            lw = 1
        elif "pbsv_pop" in sample:
            c = "#F4D94A"  # Dark yellow
            st = "-"
            lw = 3
    ...
    for i in range(num_columns):
        ...
        ymin, ymax = np.min(column_data), np.max(column_data)
        range_expand = (ymax - ymin) * 0.01
        final_min = ymin
        final_max = ymax + range_expand
...
def evaluate_subdirs(subdirs: list[str], parent_dir: str, plot_type: str, plot_title: str, save_fig: bool):
...
if __name__ == "__main__":
    ...
    evaluate_subdirs(subdirs=[args.input_dir] if args.input_dir is not None else None, parent_dir=args.parent_dir, plot_type=args.type, plot_title=args.plot_title, save_fig=args.save_fig)
