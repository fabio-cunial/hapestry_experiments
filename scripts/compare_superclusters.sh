...
function DownloadSuperclusters() {
    ...
    for SUBMISSION_ID in 1b4c026f-b753-40da-9d17-401b342008f6  60fcf740-4ed8-4fd6-aebc-b5288fd77001  ea3d9c0c-d121-4ce2-941d-982d47d57375 ; do
        gcloud storage cp ${REMOTE_ROOT}/${SUBMISSION_ID}/VcfdistEvaluationPrime/'*/call-Impl/*_superclusters.tsv' ${CURRENT_DIR}
    done
...
function CompareSuperclusters() {
    ...
    PREFIX="${LOCAL_ROOT}/${MIN_SV_LENGTH}/${COVERAGE}"
    HAPESTRY_SUPERCLUSTERS_TSV="${PREFIX}/hapestry/${SAMPLE_ID}_superclusters.tsv"
    KANPIG_SUPERCLUSTERS_TSV="${PREFIX}/kanpig/${SAMPLE_ID}_superclusters.tsv"
    FRACTIONS_FILE="${PREFIX}/fractions.csv"
    ...
    cat ${HAPESTRY_SUPERCLUSTERS_TSV} | tail -n +2 | awk 'BEGIN { FS="\t"; OFS="\t"; } { \
        ...
        printf("%s\t%d\t%d\t%d\n",$1,$3,$4,min); \
    }' > ${PREFIX}/${SAMPLE_ID}_hapestry.bed
    cat ${KANPIG_SUPERCLUSTERS_TSV} | tail -n +2 | awk 'BEGIN { FS="\t"; OFS="\t"; } { \
        if ($10<$11) min=$10; \
        else min=$11; \
        printf("%s\t%d\t%d\t%d\n",$1,$3,$4,min); \
    }' > ${PREFIX}/${SAMPLE_ID}_kanpig.bed
    N_SUPERCLUSTERS_HAPESTRY=$(wc -l < ${PREFIX}/${SAMPLE_ID}_hapestry.bed)
    N_SUPERCLUSTERS_KANPIG=$(wc -l < ${PREFIX}/${SAMPLE_ID}_kanpig.bed)

    # Building clusters of overlapping or adjacent records
    java CompareSuperclusters ${PREFIX}/${SAMPLE_ID}_hapestry.bed ${N_SUPERCLUSTERS_HAPESTRY} ${PREFIX}/${SAMPLE_ID}_kanpig.bed ${N_SUPERCLUSTERS_KANPIG} 1> ${PREFIX}/${SAMPLE_ID}_hapestry_kanpig.tsv 2>> ${FRACTIONS_FILE}

    # Basic consistency check
    java CompareSuperclusters ${PREFIX}/${SAMPLE_ID}_kanpig.bed ${N_SUPERCLUSTERS_KANPIG} ${PREFIX}/${SAMPLE_ID}_hapestry.bed ${N_SUPERCLUSTERS_HAPESTRY} > ${PREFIX}/${SAMPLE_ID}_tmp.tsv
    cut -f 1-3 ${PREFIX}/${SAMPLE_ID}_hapestry_kanpig.tsv > ${PREFIX}/${SAMPLE_ID}_tmp1.txt
    cut -f 1-3 ${PREFIX}/${SAMPLE_ID}_tmp.tsv > ${PREFIX}/${SAMPLE_ID}_tmp2.txt
    diff --brief ${PREFIX}/${SAMPLE_ID}_tmp1.txt ${PREFIX}/${SAMPLE_ID}_tmp2.txt
    rm -f ${PREFIX}/${SAMPLE_ID}_tmp.tsv ${PREFIX}/${SAMPLE_ID}_tmp1.txt ${PREFIX}/${SAMPLE_ID}_tmp2.txt

    # Extracting pairs of distances for plotting
    cut -f 4,5 ${PREFIX}/${SAMPLE_ID}_hapestry_kanpig.tsv > ${PREFIX}/${SAMPLE_ID}_hapestry_kanpig_pairs.tsv
}
...
#rm -rf ${LOCAL_ROOT} ; mkdir -p ${LOCAL_ROOT}
#DownloadSuperclusters ${LOCAL_ROOT}

FRACTIONS_FILE="${LOCAL_ROOT}/10bp/8x/fractions.csv"
rm -f ${FRACTIONS_FILE}
for SAMPLE_ID in ${SAMPLES}; do
    CompareSuperclusters ${SAMPLE_ID} 10bp 8x
done

FRACTIONS_FILE="${LOCAL_ROOT}/10bp/32x/fractions.csv"
rm -f ${FRACTIONS_FILE}
for SAMPLE_ID in ${SAMPLES}; do
    CompareSuperclusters ${SAMPLE_ID} 10bp 32x
done

FRACTIONS_FILE="${LOCAL_ROOT}/50bp/8x/fractions.csv"
rm -f ${FRACTIONS_FILE}
for SAMPLE_ID in ${SAMPLES}; do
    CompareSuperclusters ${SAMPLE_ID} 50bp 8x
done

FRACTIONS_FILE="${LOCAL_ROOT}/50bp/32x/fractions.csv"
rm -f ${FRACTIONS_FILE}
for SAMPLE_ID in ${SAMPLES}; do
    CompareSuperclusters ${SAMPLE_ID} 50bp 32x
done
